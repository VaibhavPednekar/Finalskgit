...
if (searchParams.has('id')) {
    ...
    document.querySelector('#playGame').addEventListener('click', async () => {
        ...
        my.id = socket.id;
        if (searchParams.has('id')) {
            document.querySelector('#gameLink').value = `${window.location.protocol}//${window.location.host}/rooms?id=${searchParams.get('id')}`;
            putPlayer(my);
        }
    ...
    document.querySelector('#createRoom').addEventListener('click', async () => {
        ...
        socket.on('newPrivateRoom', (data) => {
            document.querySelector('#gameLink').value = `${window.location.protocol}//${window.location.host}/rooms?id=${data.gameID}`;
            putPlayer(my);
        });
    });
}
